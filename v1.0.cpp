...
int min_layer(int deep, const Point father, int f, int computerside)
{
	...
	for (int i = 0; i < t.size()-1; i=i+2)
	{
		...
		int temp = max_layer(deep-1, t[i], !f, computerside);
...
int max_layer(int deep, const Point father, int f, int computerside)
{
	...
	for (int i = 0; i < t.size()-1; i=i+2)
	{
		...
		virtualBoard[t[i].x][t[i].y] = f;//落子
		virtualBoard[t[i + 1].x][t[i + 1].y] = f;
		int temp = min_layer(deep - 1, t[i], !f, computerside);
		//upDataAB(t[i], temp, f);//更新AB值
		if (temp > re) re = temp;
		/*if (isCut(t[i]))
			break;*/
...
void decide(int computerside, Step& myStep) {
	...
	getVaildPoint(v);
	int best = MIN;
	int deep = 4;//没有剪枝的情况下，先搜索4层看一下计算时间
	vector<Point> vbest;
	...
	for (int i = 0; i < len-1; i=i+2) {
		...
		int tmp = min_layer(deep - 1, v[i], !computerside, computerside);
		if (tmp == best) {
			vbest.push_back(v[i]);
