...
string type[22] = { "oxxxxx+","+xxxxxo","+xxxxx+","+xxxx+","+x+xxx+" ,"+xx+xx+","+xxx+x+", "oxxxx++","++xxxxo", "xxx+xx", "xx+xxx", "xx++xx" ,"oxxx+x+","+x+xxxo"  ,"oxx+xx+","+xx+xxo","ox+xxx+","+xxx+xo", "x+xxxx","xxxx+x","x++xxx","xxx++x" };
...
void mustWin(int fake_Board[19][19], Step& myStep, int op)
{
	...
	case 8://不同于mustDone
	{
		...
		myStep.second.y = start_Y + (end - 2) * num2;
	}; break;
	case 9://不同于mustDone
	{
		myStep.first.x = start_X + (end - 6) * num1;
	...
	case 17:
	{
		...
		myStep.first.y = start_Y + (end - 5) * num2;
		myStep.second.x = start_X + (end - 1) * num1;
		myStep.second.y = start_Y + (end - 1) * num1;
	}; break;
	case 18:
	{
...
void decide(int computerside, Step& myStep) {
	...
	if (sf && !sc) {//必须堵一手的情况
		...
		//堵一手之后判断，是否还有必须堵的地方
		tx = myStep.first.x;//先保存第一个点的坐标
		ty = myStep.first.y;
		myStep.first.x = -1;
		myStep.first.y = -1;
		myStep.second.x = -1;
		myStep.second.y = -1;
		mustDone(virtualBoard, myStep, computerside);//在这里会对myStep.first更新，所以需要先保存该值
		sf = (myStep.first.x != -1) && (myStep.first.y != -1);// sf = step.first
		sc = (myStep.second.x != -1) && (myStep.second.y != -1);// sc = step.second
		if (!sf && !sc) {
			myStep.first.x = tx;
			myStep.first.y = ty;
			//没有更新myStep，则到for循环中去决定第二步
		}
		if (sf && !sc) {//更新了myStep，就直接给第二步赋值就行
