...
/*******************待改进********************/
// x表示对方棋子，o表示我方棋子，以下7种情况为必须堵的情况
//应该新增必须堵一手 "oxxxx+","+xxxxo" 已增加
//"xxx+xx", "xx+xxx", "xx++xx"  已增加
string type[12] = { "oxxxxx+","+xxxxxo","+xxxxx+","+xxxx+","+x+xxx+" ,"+xx+xx+","+xxx+x+", "oxxxx+","+xxxxo", "xxx+xx", "xx+xxx", "xx++xx" };
...
void decide(int computerside, Step& myStep) {
	...
	int myY2 = 0;
	int myMaxScore = 0;
	for (int i = 0; i < 19; i++) //确定第一个点
		for (int j = 0; j < 19; j++)
		{
			if (virtualBoard[i][j] == EMPTY)
			{
				virtualBoard[i][j] = computerside;
				double temMyScore1 = CalculateGrade(virtualBoard, i, j, computerside);
				for (int m = 0; m < 19; m++)
					for (int n = 0; n < 19; n++)
					{
						if (virtualBoard[m][n] == EMPTY)
						{
							double temMyScore2 = CalculateGrade(virtualBoard, m, n, computerside);
							if (temMyScore1 + temMyScore2 > myMaxScore)
							{
								myX1 = i;
								myY1 = j;
								myX2 = m;
								myY2 = n;
								myMaxScore = temMyScore1 + temMyScore2;
							}
						}
					}
	...
	if (myMaxScore >= 100000) {//如果存在必赢的情况，给myStep赋值之后就直接返回
		myStep.first.x = myX1;
		myStep.first.y = myY1;
		myStep.second.x = myX2;
		myStep.second.y = myY2;
		return;
	}
